...
public class DBAdapter {
    ...
    public boolean insertWeatherCondition (String lat, String lng, String temperature, String wind, String humidity) {
        try {
            //Call data base
    ...
    public WeatherConditions getWeatherConditions(int id) {
        ...
        if(cursor != null) {
            cursor.moveToFirst();
            conditions = new WeatherConditions(cursor.getString(0), cursor.getString(1),cursor.getString(2), cursor.getString(3), cursor.getString(4));
            cursor.close();
        }
        db.close();
        return conditions;
    ...
    public ArrayList<WeatherConditions> getAllWeatherConditions() {
        ...
        if (cursor != null) {
            ...
            for (int i = 0; i < cursor.getCount(); i++) {
                weatherConditions.add(new WeatherConditions(cursor.getString(0), cursor.getString(1),cursor.getString(2), cursor.getString(3), cursor.getString(4)));
                cursor.moveToNext();
            }
            cursor.close();
        }
